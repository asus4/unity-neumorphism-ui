...
Shader "UI/Neumorphism Photo"
{
    ...
    SubShader
    {
        ...
        {
            half3 sample = tex2D(_SDFTex, IN.texcoord);

            float2 sdfUnit = _PixelRange / _SDFTex_TexelSize.zw;
            float sdf = msdf(_SDFTex, IN.texcoord);
            ...
            float3 normal = msdf_normal(_SDFTex, _SDFTex_TexelSize, IN.texcoord);
            // Inner shadow
            if(_Height < 0)
            {
